...
export const birdPlan: Plan = (b, has, fit, identity) => {
  ...
  for (const side of [-1, 1]) birdLeg(parts, b, side, gadfly ? 0.7 : 0.6);
  hips(parts, b, 0.85);

  parts.push(
    shaped('centre_torso', PROFILES.hull, [gadfly ? -b.long * 0.06 : 0, 0, 0],
      [b.long * (gadfly ? 0.9 : 1), b.tall, b.wide], 'plate', b.pitch),
    part('centre_torso', 'box', [-b.long * 0.34, b.tall * 0.4, 0],
      [b.long * 0.34, b.tall * 0.3, b.wide * 0.6], 'deep', b.pitch),
  );
  for (const side of [-1, 1]) {
    const location = side < 0 ? 'left_torso' : 'right_torso';
    const nacelleOut = gadfly ? 0.9 : 0.84;
    parts.push(
      shaped(location, PROFILES.pod,
        [-b.long * 0.02, b.tall * (gadfly ? 0.1 : 0.18), side * b.wide * nacelleOut],
        [b.long * (gadfly ? 0.54 : 0.66), b.tall * (gadfly ? 0.42 : 0.5), b.wide * 0.36],
        'plate', b.pitch),
      part(location, 'box',
        [b.long * (gadfly ? 0.25 : 0.28), b.tall * (gadfly ? 0.1 : 0.18), side * b.wide * nacelleOut],
        [0.07, b.tall * (gadfly ? 0.24 : 0.3), b.wide * 0.24], 'deep', b.pitch),
    );
  ...
  const headX = b.long * (gadfly ? 0.68 : 0.56);
  const headY = b.tall * (gadfly ? 0.28 : 0.34);
  parts.push(
    ...
    shaped('head', PROFILES.wedge, [headX, headY, 0],
      [gadfly ? 0.64 : 0.5, gadfly ? 0.34 : 0.32, gadfly ? 0.44 : 0.4], 'deep', b.pitch),
    part('head', 'box', [headX + (gadfly ? 0.12 : 0.06), headY + 0.03, 0],
      [gadfly ? 0.2 : 0.16, 0.14, gadfly ? 0.3 : 0.26], 'glass', b.pitch),
    part('head', 'box', [headX - 0.06, headY + 0.18, 0],
      [gadfly ? 0.4 : 0.3, 0.07, gadfly ? 0.38 : 0.34], 'plate', b.pitch),
  );
  ...
  for (const side of [-1, 1]) {
    ...
    parts.push(
      ...
      part(location, 'limb',
        [b.long * (gadfly ? 0.18 : 0.12), -b.tall * (gadfly ? 0.1 : 0.16), side * b.shoulder],
        [gadfly ? 0.21 : 0.24, b.tall * (gadfly ? 0.42 : 0.5), gadfly ? 0.18 : 0.2], 'plate'),
    );
  }
